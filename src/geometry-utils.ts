 ...
 */
export function verticesToTypedArray(vertices: Vec3[]): Float32Array {
 ...
 */
export function typedArrayToVertices(flat: Float32Array): Vec3[] {
  const vertices: Vec3[] = [];
 ...
 */
export function trianglesToTypedArray(triangles: Triangle[]): Uint32Array {
 ...
 */
export function typedArrayToTriangles(flat: Uint32Array): Triangle[] {
 ...
 */
export function deduplicateVertices(
  vertices: Vec3[] | Float32Array,
  triangles: Triangle[] | Uint32Array,
  epsilon = 1e-6
): { vertices: Vec3[]; triangles: Triangle[]; stats: { original: number; deduplicated: number; reduction: string } } {
 ...
 */
export function hashGeometry(vertices: Vec3[] | Float32Array, triangles: Triangle[] | Uint32Array): string {
  const verticesFlat = vertices instanceof Float32Array 
    ? vertices 
    : verticesToTypedArray(vertices);
 ...
 */
export function estimateMemoryUsage(vertices: Vec3[] | Float32Array, triangles: Triangle[] | Uint32Array): {
  ...
  const currentBytes = vertices instanceof Float32Array 
    ? vertices.byteLength + (triangles as Uint32Array).byteLength + arrayOverhead

...
export interface SceneJSON {
  ...
  basematerials?: Array<{ name: string; displaycolor: string }>;
  objects?: Array<
    | {
        type: 'mesh';
        name?: string;
        ...
        triangleMaterials?: Array<number>; // Array of material indices per triangle
      }
    | {
        type: 'components';
        name?: string;
        components: Array<{ objectIndex: number; transform?: [number, number, number, number, number, number, number, number, number, number, number, number]; path?: string }>;
      }
  >;
  build?: Array<{ objectIndex: number; transform?: [number, number, number, number, number, number, number, number, number, number, number, number] }>;
  external?: Array<{
    path: string;
    name: string;
    vertices: Array<[number, number, number]>;
    triangles: Array<[number, number, number]>;
    materialIndex?: number;
    buildItem?: { transform?: [number, number, number, number, number, number, number, number, number, number, number, number] };
  }>;
}
...
export function validateSceneJSON(obj: unknown): { ok: boolean; errors?: string[] } {
  const ok = validateFn(obj);
  if (ok) return { ok: true };
  const errors = (validateFn.errors || []).map((e: any) => `${e.instancePath || '(root)'} ${e.message}`);

...
import { validateSceneJSON } from './validate.js';
/**
 ...
 * ```
 */
export async function generate3MF(scene, options = {}) {
    const { production = false, validate = true, strictValidation = true, } = options;
    ...
    // Set configuration
    if (scene.unit)
        model.setUnit(scene.unit);
    if (scene.lang)
        model.setLanguage(scene.lang);
    if (production || scene.production)
    ...
    for (const obj of scene.objects ?? []) {
        if (obj.type === 'mesh') {
            const material = obj.materialIndex !== undefined
                ...
                : undefined;
            const id = model.addMesh(obj.vertices, obj.triangles, { name: obj.name, material });
            objectIds.push(id);
    ...
    if (scene.build && scene.build.length > 0) {
        ...
        for (const item of scene.build) {
            model.addBuildItem(objectIds[item.objectIndex], item.transform);
        }
    }
    // Add external models
    for (const ext of scene.external ?? []) {
        const material = ext.materialIndex !== undefined
            ...
            : undefined;
        const ref = model.addExternalMesh(ext.path, ext.name, ext.vertices, ext.triangles, { material });
        model.addExternalBuildItem(ref.objectid, ref.path, ext.buildItem?.transform);
    }
    return model.to3MF();
}
